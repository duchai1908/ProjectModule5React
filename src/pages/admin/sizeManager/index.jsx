...
import { deleteSize, getAllSize } from "../../../services/sizeService";
import AddSizeModal from "./formAddSize";
import { changePageSize } from "../../../redux/slices/sizeSlice";
import UpdateSizeModal from "./formUpdateSize";
...
export default function SizeManager() {
  ...
  const handleChangePage = async (page, pageSize) => {
    ...
    dispatch(changePageSize(page - 1)); // Adjusting for 0-based index
  };
  ...
  const handleDelete = async (currentColorDD) => {
    await dispatch(deleteSize(currentColorDD.id));
    // console.log("Product detail deleted successfully");
  };

  const [currentSize, setCurrentSize] = useState(null);
  // Handle edit product detail
  const handleEdit = async (currenSizeLL) => {
    setCurrentSize(currenSizeLL);
      ...
      <div>
        <div className="container mx-auto p-6 max-w-6xl">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-[24px] font-bold">Danh sách kích thước</h1>
                ...
                <table className="w-full table-auto">
                  <thead>
                    <tr className="bg-gray-200">
                      ...
                      <th className="px-4 h-11 text-left">Kích cỡ</th>
                    ...
                    {data.map((item, index) => {
                      ...
                      return (
                        <tr className="border-b " key={item.id}>
                          ...
                          <td className="px-4 h-11">{item.size}</td>
